...
class CourseReminderDeviceTest {
    ...
    @Test fun realAlarmCountsDownChangesPhaseDismissesAndStopsService(): Unit = runBlocking {
        ...
        val settings = NotificationSettings(live = true, afterClass = true, beforeMinutes = 1, afterMinutes = 1)
        ...
        try {
            ...
            evidence("countdown")
            assertTrue(first.notification.extras.getBoolean(Notification.EXTRA_CHRONOMETER_COUNT_DOWN))
            withTimeout(5000) { while (!CourseLiveService.running) delay(100) }
            ...
            withTimeout(100_000) {
                while (manager.activeNotifications.any { it.notification.extras.getString(Notification.EXTRA_TITLE)?.startsWith(title) == true } || CourseLiveService.running) delay(100)
            }
    ...
    private suspend fun restoreFixture(context: android.content.Context, container: AppContainer) {
        ...
        val testSettings = NotificationSettings(live = true, afterClass = true, beforeMinutes = 1, afterMinutes = 1)
