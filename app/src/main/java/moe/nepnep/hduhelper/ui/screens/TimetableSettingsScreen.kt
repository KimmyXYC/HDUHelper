...
fun TimetableSettingsScreen(state: TimetableUiState, onChange: (TimetableSettings) -> Unit, onCampus: (String?) -> Unit, modifier: Modifier = Modifier,
    ...
    Column(modifier.verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        Card(Modifier.fillMaxWidth()) {
            ...
            SwitchPreference(s.showSaturday, { onChange(s.copy(showSaturday = it)) }, "显示周六", modifier = Modifier.testTag("setting_saturday"))
            SwitchPreference(s.showSunday, { onChange(s.copy(showSunday = it)) }, "显示周日", modifier = Modifier.testTag("setting_sunday"))
