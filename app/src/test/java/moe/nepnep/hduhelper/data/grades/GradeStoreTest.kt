...
class GradeStoreTest {
    ...
    @Test fun encryptionIsolationCorruptionAndClear() {
        ...
        val store=EncryptedGradeStore(dir) { key }
        val first=snapshot(grade("a")).copy(account="synthetic-account")
        store.save(first)
        ...
        assertEquals(first,EncryptedGradeStore(dir) { key }.load(first.account,term))
