...
运行 `NeoSessionTest`、`NeoStoreTest`、`ElectricViewModelTest` 和 SSO 协议测试；`ElectricUiTest`、`ElectricNavigationTest` 验证展示、确认和导航。安装同签名的主应用及测试 APK 后，执行 `python3 tools/live-auth-smoke.py --serial DEVICE_SERIAL --electric`，交互输入测试账号；该测试只读取登录、绑定、余额、历史及宿舍选项，不修改真实绑定、不保存真实响应。真实更换、解绑通过界面手动验收。
