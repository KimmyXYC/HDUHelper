...
调研日期：2026-09-07（Asia/Shanghai）。范围：用户授权账号的登录、个人课表、菜单及相关校历、作息查询。第 1–8 节保留实现前的协议调研记录；课表功能已于同日实现，当前实现与验证结果见第 9 节。
...
以下保留调研时的接入方案和基线限制；当前已落地的改动见第 9 节。

### 7.1 复用认证仓库，增加受限的教务服务定义

调研时 [AuthRepository.kt](../app/src/main/java/moe/nepnep/hduhelper/data/auth/AuthRepository.kt) 已提供共享恢复、`ServiceIdentity`、账号代次检查和失效通知。[HduAuthApi.kt](../app/src/main/java/moe/nepnep/hduhelper/data/auth/HduAuthApi.kt) 当时存在以下两个接入限制：
...
最初调研只新增文档；后续功能实现的构建与测试记录见第 9 节。
...
## 9. 已实现功能与验证（2026-09-07）

课表页面现已使用原生 Compose 实现。新增 `data/timetable` 中的 `JwApi`、`TimetableParser`、`TimetableRules`、`TimetableRepository` 和加密缓存，以及独立 `TimetableViewModel`。现有认证仓库已允许明确配置的教务 service，仍在发出业务回调前截取 ticket；SSO Cookie 不会导出到教务域名。`JwApi` 建立自己的 CookieJar，取得 route 后通过 HTTPS 消费已确认的桥接路径。

整学期课表、周次日期、日时段和逐节作息均通过本报告的接口获取。缓存按账号、学期加密并原子写入；教务 Cookie 不落盘。失败保留最近成功数据，只有授权失效才恢复一次并重试，权限或未开放不会计入密码错误。退出/换号会使正在进行的请求失效并清理缓存。

页面包含假期倒计时、左右滑周、学期切换、下拉刷新、其他安排列表和完整课程详情。“我的 → 课表设置”提供五个显示开关及作息校区选择。时间轴的自动校区按去重课程/教学班数量决定，手动选择按账号、学期保存；详情独立使用该排课所属校区的时间。

课程结课以正在浏览的周和整门课程/教学班的最后授课周判断。重叠按本周、最近非本周、最近已结课排序；同距离时未来优先，再按稳定标识排序。总览仅绘制各排课未被优先课程遮挡的部分，详情切换使用未经过显示开关过滤的数据。课程卡片在午休、晚休处分段，保留原始节次范围供详情显示。

验证覆盖：

- JVM：CAS HTTP 服务标识与 HTTPS 传输分离、完整桥接、Cookie 路径隔离、901、权限拒绝、账号/学期校验、单双周与分段节次、全课程结课、部分重叠、隐藏详情、跨年校历、多校区作息、加密缓存损坏与账号隔离、恢复重试、离线显示和迟到响应。
- 设备：假期页与教学周滑动、学期选择、周末开关、五个开关、手动作息校区、完整节次滚动、隐藏课程详情、两套作息对应的详情时间、浅深主题和密集长名称的合成截图。
- 真实账号：原生密码登录一次后复用 SSO 获取当前与往期课表；实际教务授权、校历、作息、详情、手动刷新、离线返回和退出清理通过。测试使用独立内存认证与缓存，不改动应用原有登录资料。

本次真实排课只涉及一个校区；多校区处理已经实现并用两套不同的模拟作息验证，没有将模拟结果表述为第二校区的实测数据。独立实验模块、调休公告整合、手动编辑、导出与提醒未包含在本次实现中。执行命令见 [README 的课表章节](../README.md#课表)。


## 考试安排接入

考试使用相同教务会话，入口为 `/jwglxt/kwgl/kscx_cxXsksxxIndex.html?gnmkdm=N358105&layout=default`。查询向同一路径 POST，带 `doType=query&gnmkdm=N358105`，表单包含 `xnm`、`xqm`、`queryModel.currentPage`、`queryModel.showCount`，其他考试筛选留空。返回 `items`、`currentPage`、`totalPage`、`totalCount`；客户端校验完整分页后再更新缓存。

业务字段读取 `kcmc`、`ksmc`、`kssj`、`cdmc`、`cdxqmc`、`zwh` 和可用的 `ksbz`。时间示例为合成值 `2026-09-19(09:15-11:15)`。无法识别的时间保留原文供用户查看，不生成闹钟。考试缓存随学期加密保存；同步失败保留旧考试且显示失败状态，空结果成功则清空。日程按日期选择已缓存学期，课表的考试显示开关独立于日程和通知。

`ExamRulesTest` 覆盖解析、分页、时间定位和开始提醒；`ExamUiTest` 使用合成安排验证日程、网格、详情及开关。设备上的考试和课程采用同一通知通道、调度与超级岛协议，但考试仅生成开始事件。
