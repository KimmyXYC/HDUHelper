...
def main():
    ...
    if args.campus_code and args.timetable:
        parser.error('Choose one of --campus-code or --timetable')
    test_timeout = 600 if args.campus_code else 300 if args.timetable else 180
    ...
    runner = subprocess.Popen(adb + ['shell', 'am', 'instrument', '-w', '-r', '-e', 'class',
        ('moe.nepnep.hduhelper.LiveTimetableTest' if args.timetable else 'moe.nepnep.hduhelper.LiveCampusCodeTest' if args.campus_code else 'moe.nepnep.hduhelper.LiveAuthTest'), '-e', 'liveAuthSocket', name,
